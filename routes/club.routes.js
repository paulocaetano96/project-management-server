...
router.post("/club", async (req, res, next) => {
  try {
    ...
    const { name, sport, primaryColor, secondaryColor, teams, members } =
      req.body;
...
router.put("/club/:id", async (req, res, next) => {
  ...
  try {
    ...
    const updatedClubDetails = await Club.findByIdAndUpdate(
      id,
      { name, sport, primaryColor, secondaryColor, teams, members },
      { new: true }
    );
